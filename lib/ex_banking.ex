defmodule ExBanking do
  ...
  def create_user user do
    ...
    case Agent.get(userstore, &( Map.get &1, user )) do
      nil ->
        Agent.update(userstore, &( Map.put_new_lazy &1, user, fn -> new_user_worker user end ))
        :ok
  ...
  def send(from_user, to_user, amount, currency) do
    result = case validate_amount(amount) do
      {:error, descr} -> {:error, descr}
  ...
  defmodule UserWorker do
    ...
    def task_get_name _server, user do
      user.name
    end
    ...
    def handle_call({:hold, amount, currency}, _from, user) do
      ...
      if new_balance[currency] < 0 do
        ...
        new_user = %{user | balance: new_balance, holds: Map.put(user.holds, hold_uuid, {amount, currency})}
